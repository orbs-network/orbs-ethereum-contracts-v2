...
contract Committee is ICommittee, ManagedContract {
	...
	function addMember(address addr, uint256 weight, bool isCertified) external override onlyElectionsContract onlyWhenActive returns (bool committeeChanged) {
		...
		if (entryPos < committee.length) {
			CommitteeMember memory evicted = committee[entryPos];
			delete membersStatus[evicted.addr];
			emit GuardianCommitteeChange(evicted.addr, getMemberWeight(evicted), getMemberCertification(evicted), false);
	...
	function _removeMember(address addr) private returns (bool committeeChanged) {
		MemberStatus memory status = membersStatus[addr];
		if (!status.inCommittee) {
			return false;
		}
		...
		if (status.pos < committeeLength - 1) {
			CommitteeMember memory last = committee[committeeLength - 1];
			committee[status.pos] = last;
			membersStatus[last.addr].pos = status.pos;
		}
		committee.pop();

		emit GuardianCommitteeChange(addr, getMemberWeight(member), getMemberCertification(member), false);
	...
	function setMaxCommitteeSize(uint8 maxCommitteeSize) public override onlyFunctionalManager /* todo onlyWhenActive */ {
		...
		while (committee.length > maxCommitteeSize) {
			(address minMemberAddress, ,) = _getMinCommitteeMember();
			_removeMember(minMemberAddress);
		}
