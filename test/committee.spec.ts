...
describe('committee', async () => {
    ...
    it('does not allow a non-ready validator to join committee even if was ready previously', async () => {
    ...
    it('validator can overtake committee member with more stake', async () => {
        const d = await Driver.new();
    ...
    it('non-committee-ready validator with more stake cannot overtake a committee member', async () => {
        const d = await Driver.new();
    ...
    it('emit committeeChanged when committee member change compliance', async () => {
        const d = await Driver.new();
    ...
    it('emit committeeChanged for new committee member with correct compliance flags', async () => {
        const d = await Driver.new();
    ...
    it('uses address as tie-breaker when stakes are equal', async () => {
        ...
        const excluded = validators.find(x => x.address == minAddress(validators.map(x => x.address))) as Participant;
        const committee = validators.filter(x => x != excluded);
        let r = await excluded.becomeValidator(stake, false, false, true);
        for (let i = 0; i < maxCommitteeSize; i++) {
    ...
    it("sets and gets settings, only functional owner allowed to set", async () => {
        ...
        await expectRejected(d.committee.setMaxCommittee(maxCommitteeSize.sub(bn(1)), {from: d.migrationOwner.address}));
    ...
    it("validates 0 < maxCommitteeSize <= 32", async () => {
       ...
       await expectRejected(d.committee.setMaxCommittee(33, {from: d.functionalOwner.address}));
       await expectRejected(d.committee.setMaxCommittee(0, {from: d.functionalOwner.address}));
    });
    ...
    it("validate constructor arguments (0 < maxCommitteeSize <= 32)", async () => {
        ...
        await expectRejected(d.web3.deploy('Committee', [0, 1]));
        await expectRejected(d.web3.deploy('Committee', [33, 1]));
    ...
    it("emits incremental committee change events", async () => {
        ...
        // evicted => committee
        let {v, r} = await d.newValidator(stake, false, false, true);
        expect(r).to.have.a.validatorCommitteeChangeEvent({
            addr: v.address,
            weight: stake,
            compliance: false,
    ...
    it("assigns rewards and emits snapshot event only if enough time has passed", async () => {
        ...
        let {r: r1, v: c} = await d.newValidator(bn(1), false, false, true);
