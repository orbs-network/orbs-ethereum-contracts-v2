...
describe('committee', async () => {
    ...
    it('does not allow more than maxCommitteeSize', async () => {
        ...
        expect(r).to.have.a.committeeSnapshotEvent({addrs: committee.map(s => s.address)});
    });
    ...
    it('does not allow a non-ready standby to join committee even if was ready previously', async () => {
        ...
        expect(r1).to.have.a.standbysSnapshotEvent({addrs: []});

        const {v: v2, r: r2} = await d.newValidator(stake - 1, false, false, true); // a ready standby
        expect(r2).to.have.a.committeeSnapshotEvent({addrs: [v1.address]});
        expect(r2).to.have.a.standbysSnapshotEvent({addrs: [v2.address]});

        let r = await v2.readyToSync(); // should now become not ready-for-committee
        expect(r).to.have.a.committeeSnapshotEvent({addrs: [v1.address]});
        expect(r).to.have.a.standbysSnapshotEvent({addrs: [v2.address]});
    ...
    it('standby can overtake committee member with more stake', async () => {
        ...
        r = await v.readyForCommittee();
        expect(r).to.have.a.committeeSnapshotEvent({addrs: committee.map(s => s.address)});
        expect(r).to.have.a.standbysSnapshotEvent({addrs: [v.address]});
    ...
    it('two non-ready-for-committee with more stake can overtake two ready-for-committee standbys', async () => {
        ...
        for (let i = 0; i < maxStandbys; i++) {
            ...
            let r = i <= 1 ? await v.readyForCommittee() : await v.readyToSync();
            expect(r).to.have.a.standbysSnapshotEvent({
                addrs: standbys.map(s => s.address),
            });
            expect(r).to.have.a.committeeSnapshotEvent({
    ...
    it('stake change does not count as a readyToSync notification', async () => {
        ...
        r = await v1.stake(1);
        expect(r).to.have.a.committeeSnapshotEvent({addrs: []});
        expect(r).to.have.a.standbysSnapshotEvent({ // due to weight change
            addrs: [v1.address],
        });
    ...
    it('sets last ready-to-sync timestamp to now for a validator who left the committee by unstaking and became a standby', async () => {
        ...
        expect(r2).to.have.a.standbysSnapshotEvent({
            addrs: [v2.address]
        });
        ...
        expect(r3).to.not.have.a.committeeSnapshotEvent();
        expect(r3).to.not.have.a.standbysSnapshotEvent();
    });
    ...
    it('emit committeeChanged/standbyChanged for new committee member/standby with correct compliance flags', async () => {
        ...
        expect(r2).to.have.a.standbysSnapshotEvent({addrs: []});

        const {v: s1, r: r3} = await d.newValidator(stake, false, true, false);
        expect(r3).to.have.a.standbysSnapshotEvent({
            addrs: [s1.address],
            compliance: [false]
        });
        expect(r3).to.have.a.committeeSnapshotEvent({
            addrs: [v1.address, v2.address],
        });

        const {v: s2, r: r4} = await d.newValidator(stake - 1, true, true, false);
        expect(r4).to.have.a.standbysSnapshotEvent({
            addrs: [s1.address, s2.address],
            compliance: [false, true]
        });
        expect(r4).to.have.a.committeeSnapshotEvent({
            addrs: [v1.address, v2.address],
        });
    ...
    it("sets and gets settings, only functional owner allowed to set", async () => {
        ...
        const maxTimeBetweenRewardAssignments = bn(current[1]);
        const maxCommitteeSize = bn(current[2]);
        const maxStandbys = bn(current[3]);
        ...
        r = await standbys[2].readyToSync();
        expect(r).to.have.a.standbysSnapshotEvent({
    ...
    it("emits increments committee change events", async () => {
        const d = await Driver.new({maxTimeBetweenRewardAssignments: 2*24*60*60});

        let stake = bn(100);

        // evicted => standby
        let {v, r} = await d.newValidator(stake, false, true, false);
        ...
        // standby compliance change
        r = await v.becomeCompliant();
        expect(r).to.have.a.validatorCommitteeChangeEvent({
            addr: v.address,
            weight: stake,
            compliance: true,
        ...
        // standby => evicted
        r = await v.unregisterAsValidator();
        expect(r).to.have.a.validatorCommitteeChangeEvent({
            addr: v.address,
            weight: stake,
            compliance: false,
    ...
    it("emits increments committee change events for two changed members - a new committee member and an evicted standby", async () => {
        const d = await Driver.new({
        ...
        const {v: c1} = await d.newValidator(bn(2), false, false, true);
        const {v: s1} = await d.newValidator(bn(1), false, true, false);

        const {r, v: c2} = await d.newValidator(bn(3), false, false, true);
        expect(r).to.have.a.validatorCommitteeChangeEvent({
            addr: c2.address,
            inCommittee: true,
            isStandby: false
    ...
    it("assigns rewards and emits snapshot event only if enough time has passed", async () => {
        ...
        let {r: r1, v: s} = await d.newValidator(bn(1), false, true, false);
        expect(r1).to.have.a.validatorCommitteeChangeEvent({
            addr: s.address,
            inCommittee: false,
            isStandby: true,
        });
        expect(r1).to.not.have.a.committeeSnapshotEvent();
        expect(r1).to.not.have.a.standbysSnapshotEvent();
        ...
        let {r: r2, v: c} = await d.newValidator(bn(1), false, false, true);
        expect(r2).to.have.a.validatorCommitteeChangeEvent({
            addr: c.address,
            inCommittee: true,
            isStandby: false,
        });
        ...
        expect(r2).to.not.have.a.stakingRewardsAssignedEvent();
        expect(r2).to.not.have.a.feesAssignedEvent();
        expect(r2).to.not.have.a.bootstrapRewardsAssignedEvent();

        await evmIncreaseTime(d.web3, maxTimeBetweenRewardAssignments);

        let r = await c.readyForCommittee();
        expect(r).to.have.a.validatorCommitteeChangeEvent({
            addr: c.address,
            inCommittee: true,
            isStandby: false,
