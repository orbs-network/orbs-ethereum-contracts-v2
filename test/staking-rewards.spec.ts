...
describe('staking-rewards', async () => {
  ...
  it("allows anyone to migrate staking rewards to a new contract", async () => {
    ...
    const newRewardsContract = await d.web3.deploy('Rewards', [d.contractRegistry.address, d.migrationManager.address, d.erc20.address, d.bootstrapToken.address], null, d.session);
    await d.contractRegistry.setContracts([contractId('rewards')], [newRewardsContract.address], [true], {from: d.registryManager.address});
